...
test('MusicData.encodedString roundtrip with 14-element string (including containedVersion)', () => {
  // format: musicId \t type \t isDeleted \t diff[0..8] \t containedVersion \t title  (14 elements)
...
test('MusicData.encodedString from legacy 13-element string outputs 14-element new format', () => {
  // 旧形式: [..., title] → 新形式: [..., containedVersion(空), title]
  ...
  expect(musicData.encodedString).toBe('91qD6DbDqi96qbIO66oboliPD8IPP6io\t0\t0\t3\t7\t11\t13\t16\t7\t11\t13\t16\t\t輪廻転生');
});

test('MusicData.encodedString includes musicId, type, isDeleted, difficulty, containedVersion, title in order', () => {
  ...
  expect(parts[12]).toBe('19'); // containedVersion = DDR_WORLD
  expect(parts[13]).toBe('Test Song');
});

test('MusicData.encodedString with null containedVersion outputs empty string before title', () => {
